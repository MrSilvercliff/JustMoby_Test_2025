...
- Написание ридми и комментариев в коде - 9 часов 12 минут _(в голове оно всегда выглядит проще, чем когда реально пытаешься написать ридми и раскидать комментарии)_
...
**ПРИМЕЧАНИЕ!** объект под курсором я оставил прозрачным **НАМЕРЕННО**, чтобы было видно, куда целимся и куда происходит дроп.
...
Вот как-то так. Надеюсь, что Вы дочитали до этого раздела :)

Задание на первый взгляд показалось не очень сложным, а когда выяснилось, что все можно сделать на канвасах, то в голове стало выглядеть еще проще. А потом я приступил к работел, потратил довольно много времени на организацию баланса, убил кучу времени на драг-н-дроп, немного утонул в деталях постройки башни, стал нервничать и тяжело пришел к тому, что надо срезать углы. Это, наверное, первое тестовое задание на моей памяти, где я не успел сделать все указанные в нем пункты.

### **Слабые стороны на мой взгляд:**

- **Не сделал все пункты и порезал часть механик**

- **Адаптация под разные разрешения** - я про нее немного забыл :( хотел сделать в самом конце, но на часах было уже 23:40 воскресенья 5 октября, я заторопился, отправил тестовое на проверку и принял решение больше его не дорабатывать. В соотношении сторон 4:3 игра выглядит сносно, все умещается на экране и только анимированный текст выходит за его пределы, но `Canvas Scale-ром` можно было бы сделать картину еще лучше.

- **Инпут** - эвенты и легаси `Input Manager` вместо современной `Input System`. А еще нет отдельного контроллера инпута для работы с тачами, поэтому собрать билд под андроид из того, что получилось, возможно игра вообще не будет работать (Да, Миша, давай, закапывай себя сам, ага) :)

- **Наличие небольшого пролага при первой DOTween анимации** - есть подозрение, что это фиксится вызовом инициализации сервисов на старте игры. (апдейт: пофиксил :) )

- **Канвас с анимациями** - настройки анимаций хранятся непосредственно в виджетах канваса, а не в отдельном конфиге в балансе игры. Пришлось так сделать, боялся не успеть.

- **Контроллер драг-н-дропа** - из него получился `God-object`, что в принципе не очень хорошо и вообще антипаттерн. Но так, наверное, получилось из-за специфики игры, т.к. почти весь геймплей происходит через драг-н-дроп

- **Скролл** - с самим скроллом все хорошо, просто в задании просили сделать так, чтобы при драге кубика не драгался сам скролл, и из-за моей попытки "учесть специфику целевой аудитории" я сам создал эту возможность. Я постарался поймать момент драга кубика через дельту по вертикали, получилось неплохо, но скролл все равно немного драгается, если тянуть кубить по диагонали. Возможно, в этом нет ничего страшного, но хочется остаться максимально честным :)

### **Сильные стороны на мой взгляд:**

- **Архитектура** - получился почти готовый плацдарм для большого приложения с кучей мини-игр внутри. 

- **Драг-н-дроп** - визуальная часть драг-н-дропа с объектом под курсором вынесена в отдельный `GameObject` и никак не пересекается с остальной частью игрового канваса. 

- **Extenhect (ex Zenject)** - наличие фреймворка в проекте было указано как важный пункт при оценивании.

Что-то слабых сторон получилось больше, чем сильных, но среди последних есть самая важная :)
