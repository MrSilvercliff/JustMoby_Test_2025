...
{
    public interface IViewController : IProjectService
    { 
    ...
    public abstract class ViewControllerBase : MonoBehaviour, IViewController
    {
        ...
        public async Task OpenView(Type viewType, IWindowSetup setup)
        {
            ...
            var viewTaskResult = true;

            if (_activeView != null)
                viewTaskResult = await CloseActiveView();
            ...
            viewTaskResult = await OpenNewView(newView, setup);
        ...
        private async Task<bool> CloseActiveView()
        {
            try 
            {
                ...
                _activeView.Controller.SetActive(false);
        ...
        private async Task<bool> OpenNewView(IViewWindow newView, IWindowSetup setup)
        {
            try
            {
                await newView.Setup(setup);
                
                newView.Controller.PrepareToOpen();
                newView.Controller.SetActive(true);
