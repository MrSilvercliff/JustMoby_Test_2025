...
using UnityEngine;
using Zenject;
using ZerglingUnityPlugins.UI.Scripts.Pools;
...
namespace _Project.Scripts.Project.Services.UI.Popups
{
    public class PopupController : PopupControllerBase
    {
        ...
        private Dictionary<Type, IZenjectFactoryMonoPool<PopupWindow>> _projectPools;
        private Dictionary<Type, IZenjectFactoryMonoPool<PopupWindow>> _scenePools;
        ...
        private async Task InitProjectPopups()
        {
            ...
            _projectPools = new Dictionary<Type, IZenjectFactoryMonoPool<PopupWindow>>();
        ...
        private async Task InitScenePopups()
        {
            if (_scenePopups == null)
            {
                ...
                _scenePools = new Dictionary<Type, IZenjectFactoryMonoPool<PopupWindow>>();
            }
        ...
        private async Task InitPopupsList(Dictionary<Type, PopupWindow> dictionary, IReadOnlyCollection<PopupWindow> popups)
        {
        ...
        private IPopupWindow GetPopup(Type popupType, Dictionary<Type, PopupWindow> popupDictionary, Dictionary<Type, IZenjectFactoryMonoPool<PopupWindow>> poolsDictionary)
        {
            ...
            if (!poolsDictionary.ContainsKey(popupType))
            {
                ...
                var newPool = new ZenjectFactoryMonoPool<PopupWindow>(popup, _popupFactory, _popupContainer);
        ...
        private Task ReturnToPool(PopupWindow popupWindow, Dictionary<Type, IZenjectFactoryMonoPool<PopupWindow>> poolsDictionary)
        {
        ...
        protected override Task OnCloseAll()
        {
            // TODO: complete close all popups
            return Task.CompletedTask;
        }
    }
}